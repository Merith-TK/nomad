-- notepad.lua - Opens Notepad on Windows
...
-- Background worker: polls every 2 seconds to check if notepad is running
function background(state)
    while true do
        if system.os() == "windows" then
            local out, _, code = shell.exec("tasklist /FI \"IMAGENAME eq notepad.exe\" /NH 2>nul")
            state.running = (code == 0 and out:find("notepad.exe") ~= nil)
        else
            state.running = false
        end
        system.sleep(2000)
...
function passive(key, state)
    if state.running then
        -- Green background when notepad is running
        return {
            color = {50, 180, 50},
            text = "NP*",
